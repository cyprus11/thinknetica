...
class Wagon
  ...
  def to_s
    wagon_type = passenger? ? 'Пассажирский' : 'Грузовой'
    "#{wagon_type} вагон, вместимостью: #{@all_places.size} #{place_type}, свободно: #{free_places_count} #{place_type}"
  end
  ...
  def place_type
    cargo? ? WAGON_PLACES_TYPES[1] : WAGON_PLACES_TYPES[0]
  end
end
